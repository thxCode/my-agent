...
Address the security report: **$ARGUMENTS**

A vulnerability report is an ordinary bug wearing a second job. The technical half is already covered, and this
command runs it through `/my-debug` → `/my-build` → `/my-ship` unchanged. What it **owns** is the other half: an
...
**Why a command and not a mode on `/my-ship`.** The advisory is opened *before* code is written, which is
...
- **Under embargo, nothing about the vulnerability leaves this machine.** The `codex` / `kimi` rescue subagents
  send the diff to a third party, so `crosscheck` is **off** for every lane of the run — say so the first time a
  lane would have offered it, and stop offering it.
- **Confirm the exact text before it leaves the machine** — the advisory body, the branch name, the commit
  message, the PR title and body, the tag, the reporter email, the publish. This command drafts, the user
...
`$ARGUMENTS` arrives in one of three shapes, and the shape settles 1d's classification:
...
| a public issue number or URL | `~/.claude/references/my-workflow/resolve-issue.md` |
...
The score is what the rest of this command is priced against: whether a CVE is worth requesting, how tight the
