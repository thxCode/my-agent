...
- **Language.** Write the ledger and every archived artifact in **English**; talk to the user in their
  ...
  `~/.claude/references/my-workflow/probe-craft.md` § 8 has the rule and the reason.
- **Source lookup.** Read/trace source: **GitNexus** (if available) → **DeepWiki** → `grep`/`find`. This command
  reads source to check a claim; it writes none.

**Boundaries.**

- **Everything this command writes stays under the project's `.claude/`.** The ledger and its round archive live
...
- **Every outward action is the user's.** A comment is posted only after they approve the exact text; closing an
  issue and filing upstream are printed for them to run. This command never sends, closes, or files. **Fetching
...
Resolve `$ARGUMENTS` per `~/.claude/references/my-workflow/resolve-issue.md` — it fixes `<owner>/<repo>` and
...
would spend a full context on days of comments to restate one file, and it is not how this command remembers
...
`~/.claude/references/my-workflow/probe-craft.md` holds the probe: the template with its `say` / `run` / `kx` /
...
```sh
# print these for the user; this command executes neither, ever
...
**`gh issue close`, `gh issue create` and `gh issue transfer` are never executed by this command — not on
