...
Shared by every `my-*` command whose selector is a GitHub issue. Upstream is always **GitHub**. The selector is
`$ARGUMENTS` minus any mode token, and it names exactly one issue in one repo.
