...
Finalize and ship the current branch: **the user's current request**
...
project conventions (target **Code Style** & **Boundaries** when present, applicable instruction files, existing test/doc structure).
...
2. **Attach a target (optional).** Resolve `the user's current request` per
   `~/.agents/skills/my-workflow/references/resolve-target.md` — **target-optional; never block on a missing one, any
...
6. **Kick off the ship-time cross-check (gated, background) — apply `crosscheck`.**
   Use the selected `crosscheck` route's status mechanism to adopt any review already in flight —
...
2. **Route by the surface:**
   - **Project e2e skill** (from the host's discovered project skill catalog) → run it **in the main loop** as designed —
     such skills may orchestrate their own subagents and gate mutating steps on user confirmation, so never wrap
     one in a subagent; its own execution policy remains authoritative.
   - **Otherwise** → delegate to the `test-worker` role (use the host's fast capability class — mechanical execution, off the main
